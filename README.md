...
**AI(예: ChatGPT)가 작성했을 가능성**이 높은지,  
**사람이 작성했을 가능성**이 높은지를  
언어적 특징 분석을 통해 점수화하는 *교육용 데모 프로젝트*입니다.

이 도구는 웹 기반 UI(index.html)와 Python 분석 엔진(gpt_killer_ko.py)로 구성되어 있으며,  
...
AI 생성 텍스트가 증가하면서 그 출처를 구분해야 할 필요성이 커졌습니다.  
...
`AI 점수 = raw_score × 10` 으로 **0~100점 스케일로 환산**합니다.
